...
async function menuju() {
  var { value: jawaban } = await swals.fire({
    title: 'bales dong hehe jangan lebih dari 100 huruf yaa', input: 'text', allowOutsideClick: false, showCancelButton: false,
  });
  if (jawaban && jawaban.length < 100) {
    window.jawaban = jawaban;
    pesanwhatsapp = jawaban;
    await swals.fire('makasih ya...', 'kirim jawabannya ke whatsApp aku', 'success');
    window.location = "https://api.whatsapp.com/send?phone=6281321878229&text=" + pesanwhatsapp;
    tombol();
  } else {
    await swals.fire('hmmm...', 'jawaban gaboleh kosong atau lebih dari 100 huruf ya'); jawab();
  }
}